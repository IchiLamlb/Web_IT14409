...
var mywiki = [
  {
    ...
    "details": [
      {
        "id": "mywiki-detail1-content1",
        "title": "Nội dung My Wiki",
        html: document.getElementById("mywiki-detail1-content1").innerHTML
      }
    ]
  }
]
...
function searchWikipedia() {
  ...
  fetch(url)
    ...
    .then(data => {
      const resultsContainer = document.getElementById('wikipedia-results');
      ...
      searchResults.forEach(result => {
        ...
        resultElement.classList.add('wiki-result');
        resultElement.innerHTML = `
          <h3>${result.title}</h3>
          <p>${result.snippet}...</p>
          <button onclick="useWikipediaContent('${result.title}')">Sử dụng nội dung này</button>
        `;
...
function useWikipediaContent(title) {
  const url = `https://en.wikipedia.org/api/rest_v1/page/summary/${title}`;

  fetch(url)
    .then(response => response.json())
    .then(data => {
      // Gắn nội dung vào textarea chỉnh sửa
      const editor = document.getElementById('manual-editor');
      editor.value = data.extract;

      // Lưu trực tiếp vào nội dung trên giao diện admin
      document.getElementById('preview-content').innerHTML = data.extract;

      // Chuyển lại về chế độ nhập thủ công
      document.getElementById('content-input-method').value = 'manual';
...
function handleInputMethodChange() {
  const method = document.getElementById("content-input-method").value;

  const manualInput = document.getElementById("manual-input");
  const wikipediaSearch = document.getElementById("wikipedia-search");
  const wikiImageSearch = document.getElementById("wiki-image-search");
  const docxUpload = document.getElementById("docx-upload");

  manualInput.classList.add("hidden");
  wikipediaSearch.classList.add("hidden");
  wikiImageSearch.classList.add("hidden");
  docxUpload.classList.add("hidden");
  ...
  switch (method) {
    case "manual":
      ...
      break;
    case "wikipedia":
      wikipediaSearch.classList.remove("hidden");
      break;
    case "image":
      wikiImageSearch.classList.remove("hidden");
      break;
    case "docx":
      docxUpload.classList.remove("hidden");
      break;
  }
}
...
function updatePreviewContent() {
  const editorData = document.getElementById('manual-editor').value;

  // Cập nhật nội dung hiển thị
  document.getElementById('preview-content').innerHTML = editorData;

  // Lưu vào mảng menu
  const id = document.getElementById('preview').parentNode.id;
  const sectionId = findSectionId(id);
  const arr = getArr(sectionId);

  const parts = id.split('-');
  const title2 = arr.find(item => item.id === `${parts[0]}-${parts[1]}`);
  const title3 = title2.details.find(item => item.id === id);

  // Cập nhật nội dung vào phần tử trong mảng
  title3.html = editorData;
}
...
function embedMap() {
  ...
  const iframe = `<iframe 
    ...
    loading="lazy" 
    allowfullscreen 
    src="https://www.google.com/maps/embed/v1/place?key=YOUR_API_KEY&q=${encodeURIComponent(query)}">
  </iframe>`;

  // Hiển thị bản đồ trong preview
  document.getElementById('map-preview').innerHTML = iframe;

  // Lưu bản đồ vào nội dung admin
  const editor = document.getElementById('manual-editor');
  editor.value = iframe;
}

function searchWikiImage() {
  const query = document.getElementById('image-query').value;
  const url = `https://en.wikipedia.org/w/api.php?action=query&generator=images&titles=${query}&gimlimit=5&prop=imageinfo&iiprop=url&format=json&origin=*`;
    ...
    .then(data => {
      ...
      Object.values(pages).forEach(page => {
        if (page.imageinfo) {
          ...
          imgElement.onclick = () => useWikiImage(imageUrl);
...
function useWikiImage(imageUrl) {
  // Gắn hình ảnh vào nội dung admin
  const editor = document.getElementById('manual-editor');
  editor.value = `<img src="${imageUrl}" alt="Wikipedia Image" />`;

  // Hiển thị hình ảnh trong preview
  document.getElementById('preview-content').innerHTML = `<img src="${imageUrl}" alt="Wikipedia Image" />`;
}
document.getElementById('file-input').addEventListener('change', (event) => {
  ...
  if (file) {
    ...
    // Hiển thị liên kết tới file
    const link = document.createElement('a');
    link.href = fileURL;
    link.textContent = `Xem file: ${file.name}`;
    link.target = '_blank';
    previewContainer.appendChild(link);

    // Gắn file vào nội dung admin
    const editor = document.getElementById('manual-editor');
    editor.value = `<a href="${fileURL}" target="_blank">${file.name}</a>`;
  }
});
function handleDocxUpload() {
  ...
  if (fileInput.files.length === 0) {
    alert("Vui lòng chọn một file DOCX!");
    return;
  }
  ...
  reader.onload = function (event) {
    ...
    mammoth
      ...
      .then((result) => {
        ...
        previewContainer.innerHTML = `<div>${result.value}</div>`;

        // Lưu nội dung vào textarea
        const editor = document.getElementById("manual-editor");
        editor.value = result.value;

        // Hiển thị nội dung vào preview-content
        document.getElementById("preview-content").innerHTML = result.value;
      })
      .catch((err) => {
        console.error("Error reading DOCX file:", err);
        alert("Có lỗi xảy ra khi đọc file DOCX!");
      });
  };
